...
import { Ticket, Clock, CheckCircle2, Users, Loader2 } from "lucide-react";
import { toast } from "@/hooks/use-toast";
import { apiJoinGroup } from "@/lib/ajo-data";
...
const JoinGroup = () => {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setSubmitted(true);
      toast({ title: "Joined group", description: "Welcome!" });
    } catch (err: unknown) {
      ...
      <div className="phone-shell flex flex-col">
        ...
        <div className="screen-pad flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-muted-foreground text-sm max-w-xs leading-relaxed mb-8">
            You’ve joined the group successfully using invite code <span className="font-bold text-foreground">{code}</span>.
          </p>
          ...
          <div className="w-full rounded-2xl bg-gradient-card border border-border p-4 shadow-soft mb-6 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left flex-1">
                ...
                <p className="text-[11px] text-muted-foreground truncate">{groupId ?? "—"}</p>
              </div>
            </div>
